...
class DadosXMLService {
    ...
    public function consultaDadosXML(string $chave_nota) {
        ...
        return array_filter([
            'ide' => is_null($xml->ide[0]) ? null : array($xml->ide[0]),
            'emit' => is_null($xml->emit[0]) ? null : array($xml->emit[0]),
            'dest' => is_null($xml->dest[0]) ? null : array($xml->dest[0]),
            'retirada' => is_null($xml->retirada[0]) ? null : array($xml->retirada[0]),
            'entrega' => is_null($xml->entrega[0]) ? null : array($xml->entrega[0]),
            'autXML' => is_null($xml->autXML[0]) ? null : array($xml->autXML[0]),
            'det' => is_null($xml->det[0]) ? null : $this->trataDetalhesNota($xml->det),
            'total' => is_null($xml->total[0]) ? null : array($xml->total[0]),
            'transp' => is_null($xml->transp[0]) ? null : array($xml->transp[0]),
            'cobr' => is_null($xml->cobr[0]) ? null : array($xml->cobr[0]),
            'pag' => is_null($xml->pag[0]) ? null : array($xml->pag[0]),
            'infIntermed' => is_null($xml->infIntermed[0]) ? null : array($xml->infIntermed[0]),
            'infAdic' => is_null($xml->infAdic[0]) ? null : array($xml->infAdic[0]),
            'exporta' => is_null($xml->exporta[0]) ? null : array($xml->exporta[0]),
            'compra' => is_null($xml->compra[0]) ? null : array($xml->compra[0]),
            'cana' => is_null($xml->cana[0]) ? null : array($xml->cana[0]),
        ]);
    }
    ...
    private function trataDetalhesNota(SimpleXMLElement $det): array {
        ...
        foreach ($det as $detalhe) {
            $arrayDetalhes[] = [
                'prod' => (array)$detalhe->prod,
                'imposto' => (array)$detalhe->imposto
            ];
        }
